...
function authorizeCommentModification(operation) {
    return async (req, res, next) => {
        ...
        const projection = operation ? { _id: 1 } : { likingUsersIds: 1, parentsIds: 1, repliesCount: 1 };

        try {
            const comment = await Comment.getComment({ _id: commentId, postId, creatorId: userId }, projection);

            if (!comment) {
                const exist = await Comment.getComment({ _id: commentId, postId }, { _id: 1 });
                if (exist) {
