...
function authorizeCommentModification(operation) {
    return async (req, res, next) => {
        ...
        const projection = operation ? { _id: 1 } : { likingUsersIds: 1, parentId: 1, postId: 1 };

        try {
            const comment = await Comment.getComment({ creatorId: userId, postId }).project(projection);

            if (!comment) {
                const exist = await Comment.getComment({ postId, _id: commentId }).project({ _id: 1 });
                if (exist) {
